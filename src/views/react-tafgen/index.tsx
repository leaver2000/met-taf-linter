...
function TafLinter() {
    ...
    const [passValue, setPass] = useState<any>(null);
    const [failValue, setFail] = useState<any>(null);

    useEffect(() => {
        try {

            const v = parser.parse(sampleTaf, {});
            setPass(v)
            setFail(null)
            // setParserValue(v)
    ...
    return (
        <>
            {!!passValue ? passValue.map(v => <JSONTree hideRoot data={JSON.stringify(v)} />) : null}
            {!!failValue ? <Fail data={failValue.stack} /> : null}
