...
export default class Validator {
     ...
     */
    _SurfaceBasedPartialObscuration(vvvv: number, range: () => ExceptionRange) {
        if (vvvv > 1000) {
    ...
    _ww_NNNhhhCC([[vvvv], ww, NNNhhhCC]: NNNhhhProps, range: () => ExceptionRange) {
        ...
        if (!NNNhhhCC.length) {
            ...
            // dont use BR with forecast visibility less than 1000M
            if (vvvv < 999 && obsc === "BR") {
                const message = 'When forecasting visiblity less than 1000m use FG';
            ...
            }//this._SurfaceBasedPartialObscuration(vvvv, range)
